...
const handler = {
  ...
  geneticTesting: async ({ dataEvent, value, valueMessage, event }) => {
    ...
    const wording = `${valueMessage} DAI ${
      event.method === 'DnaSampleResultReady' ? 'for completeing the requested test' : 'as quality control fees'
    } for ${formatedHash}.`;
    return { data, id, params, wording };
  },
  ...
  services: async ({ dataEvent, value, valueMessage }) => {
    ...
    const notifications = JSON.parse(
      localStorage.getLocalStorageByName(`LOCAL_NOTIFICATION_BY_ADDRESS_${localStorage.getAddress()}_lab`),
    );
