...
export async function queryServicesByMultipleIds(api: ApiPromise, serviceIds: string[]): Promise<Array<Service>> {
...
export async function queryServicesByMultipleIdsArray(api: ApiPromise, serviceIds: string[]): Promise<Array<Service>> {
  const services: Array<Service> = new Array<Service>();
  for (const id in serviceIds) {
    services.push(await queryServiceById(api, serviceIds[id]));
  }
